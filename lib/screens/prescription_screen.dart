...
class _PrescriptionScreenState extends State<PrescriptionScreen> {
  ...
  List<PrescriptionPayload> _consultations = [];
  ...
  Future<void> _loadPrescriptions() async {
    ...
    try {
      final PrescriptionResponseNew? response = await _apiService.getPrescription(context,
      ...
      if (response != null &&
        ...
        if (response.payLoad!.isEmpty) {
          ...
          setState(() {
            _consultations.addAll(response.payLoad!);
            if (response.payLoad!.length < _pageSize) {
              _isLastPage = true;
            } else {
              _pageNumber++;
            }
        ...
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(response?.statusMessage?.join(', ') ?? 'Failed to load prescriptions.')));
        setState(() {
          _errorMessage = response?.statusMessage!.join(', ');
