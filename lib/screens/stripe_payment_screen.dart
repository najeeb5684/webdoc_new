...
class _StripePaymentScreenState extends State<StripePaymentScreen> {
  ...
  String _packagesuccessUrl = "https://appointment.webdoc.com.pk/payment-success";
          ...
          ..setNavigationDelegate(
            NavigationDelegate(
              ...
              onNavigationRequest: (NavigationRequest request) {
                if (request.url.startsWith(_packagesuccessUrl)) {
  ...
  void _extractSessionId(String url) {
    Uri uri = Uri.parse(url);
    _sessionId = uri.queryParameters['session_id'];
    print("Extracted Session ID: $_sessionId");
  }
  ...
  Future<void> _activatePackageAndLogin(ApiService apiService) async {
    try {
      ...
      if (activatePackageResponse != null &&
        ...
        await _callPackagePaymentSuccessAPI();
  ...
  Future<void> _callPackagePaymentSuccessAPI() async {
    final String? userId = SharedPreferencesManager.getString('id');
    final String apiUrl = '${ApiService.irfanBaseUrl}stripe/payment-success?session_id=$_sessionId&profile_id=$userId&package_id=${widget.packageId}';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        print("Package payment success API called successfully");
        print("API Response: ${response.body}");
      } else {
        print("Failed to call package payment success API: ${response.statusCode}");
        print("API Response: ${response.body}");
      }
    } catch (e) {
      print("Error calling package payment success API: $e");
    }
  }
  ...
  Future<void> _bookSlot(ApiService apiService) async {
    ...
    try {
      ...
      Navigator.of(context).pop();
      // Debugging: Check if the response is null

      if (bookSlotResponse == null) {
        print("ERROR: bookSlotResponse is NULL!");
        _showErrorDialog("API call failed to return a response.");
        return; // Exit the function to prevent further errors
      }

      // Print the raw response for debugging - VERY IMPORTANT
      print("Raw bookSlotResponse: ${bookSlotResponse.toString()}");

      // Debugging: Check the status code
      print("statusCode: ${bookSlotResponse.statusCode}"); // Print status code
      print("statusMessage: ${bookSlotResponse.statusMessage}"); // Print status code
      print("payLoad: ${bookSlotResponse.payLoad}"); // Print status code
      if (bookSlotResponse != null && bookSlotResponse.statusCode == 1) {
        // Call appointment payment success API
       final appointmentId=bookSlotResponse.payLoad!.appointmentId;
        await _callAppointmentPaymentSuccessAPI(appointmentId);
  ...
  Future<void> _callAppointmentPaymentSuccessAPI(int? appointmentId) async {
    final String? userId = SharedPreferencesManager.getString('id');
    final String apiUrl = '${ApiService.irfanBaseUrl}stripe/appointment-payment-success?session_id=$_sessionId&profile_id=$userId&doctor_id=${widget.doctorId}&appointmentId=$appointmentId&paymentMethod=stripe';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        print("Appointment payment success API called successfully");
        print("API Response: ${response.body}");
      } else {
        print("Failed to call appointment payment success API: ${response.statusCode}");
        print("API Response: ${response.body}");
      }
    } catch (e) {
      print("Error calling appointment payment success API: $e");
    }
  }
  String generateRandomTransactionId() {
    final random = Random();
    // Ensure the number is exactly 6 digits by adding 100000 to the random 5-digit number
