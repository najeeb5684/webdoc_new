...
class _SpecialistDoctorProfileScreenState
  ...
  Future<void> _processPayment(String walletBalance, String? couponCode, String? discountedFee) async {
    ...
    // Calculate discounted fee amount if it exists
    final discountedAmount = discountedFee != null ? int.tryParse(discountedFee) ?? 0 : 0;
    final adjustedConsultationFee = consultationFee - discountedAmount;

    if (int.parse(walletBalance) >= adjustedConsultationFee) {
      // Book with wallet
      ...
      if (int.parse(walletBalance) > 0) {
        ...
        final remainingAmount = adjustedConsultationFee - int.parse(walletBalance);
        ...
        _navigateToPaymentScreen(adjustedConsultationFee, couponCode: couponCode);
 ...
 /* Future<void> _processPayment(String walletBalance, String? couponCode, String? discountedFee) async {
  ...
  Future<void> _bookAppointmentWithWallet({String? couponCode, String? discountedFee}) async {
    ...
    try {
      ...
      // Safely close the initial loading dialog
      ...
      if (bookSlotResponse != null && bookSlotResponse.statusCode == 1) {
        // Call the appointment payment success API
        // Show a second loading dialog while calling _callAppointmentPaymentSuccessAPI
        showDialog(
          context: currentContext,
          barrierDismissible: false,
          builder: (BuildContext context) {
        ...
        try {
          ...
          await _callAppointmentPaymentSuccessAPI(bookSlotResponse.payLoad?.appointmentId);
        } finally {
          // After _callAppointmentPaymentSuccessAPI is complete (success or fail),
          // close the second loading dialog and show the final success dialog
          if (Navigator.canPop(currentContext)) {
  ...
  Future<void> _callAppointmentPaymentSuccessAPI(int? appointmentId) async {
    ...
    final String apiUrl = '${ApiService.irfanBaseUrl}stripe/appointment-payment-success?session_id=wallet&profile_id=$userId&doctor_id=${widget.doctor.docid}&appointmentId=$appointmentId&paymentMethod=wallet';
