...
import '../models/user_package_response.dart'; // Import PackageResponse
...
class ApiService {
  ...
  static const String saveFeedbackEndpoint = 'SaveFeedback';
  static const String getConsultationsByUserIdEndpoint =
...
//Show No Internet connection Dialogue
  ...
  Future<UserCheckResponseNew?> userCheck(
    ...
    try {
      final url = Uri.parse('$baseUrl$userCheckEndpoint');
  ...
  Future<LoginResponse?> login(
    ...
    try {
      final url = Uri.parse('$baseUrl$patientLoginEndpoint');
      final headers = {'Content-Type': 'application/json'};
...
// Password Reset API Call
  Future<ChangePasswordResponse?> resetPassword(BuildContext context, String mobileNumber, String pin) async {
    ...
    final Uri apiUrl = Uri.parse('$baseUrl$passwordResetEndpoint');
  ...
  Future<UserCheckResponse?> saveFeedback({
    ...
    try {
      final response = await http.post(
        Uri.parse('$baseUrlFahad$saveFeedbackEndpoint'),
      ...
      if (response.statusCode == 200) {
        final decodedJson = jsonDecode(response.body);
        return UserCheckResponse.fromJson(decodedJson);
      } else {
  ...
  Future<Profile?> getPatientProfile(BuildContext context, String id) async {
    ...
    try {
      final url = Uri.parse('$baseUrl$getPatientProfileEndpoint'); // Assumes baseUrl and getPatientProfileEndpoint are defined elsewhere
      final headers = {
      ...
      final response = await http.get(
          Uri.parse('$baseUrl$getPatientProfileEndpoint?patient_profile_id=$id'),
          headers: headers);
  ...
  Future<UserCheckResponseNew?> updatePatientProfile({
    ...
    try {
      final url = Uri.parse('$baseUrl$updatePatientProfileEndpoint');
      final headers = {'Content-Type': 'application/json'};
      final body = json.encode({
  ...
  Future<SendOtpResponse?> sendOtp(BuildContext context, String mobileNumber) async {
    ...
    final Uri url = Uri.parse(baseUrl + sendOtpEndpoint); // Correct URL construction
...
// Verify OTP API Call
  Future<VerifyOtpResponse?> verifyOtp(BuildContext context, String mobileNumber, String otp) async {
    ...
    final Uri url = Uri.parse(baseUrl + verifyOtpEndpoint);  //Correct URL Construction
  ...
  Future<RegistrationResponse?> registerUser(BuildContext context, String mobileNumber, String name, String pin,String email) async {
    if (!await isInternetAvailable()) {
      showNoInternetDialog(context);
      return null;
    }

    final Uri apiUrl = Uri.parse('$baseUrl$registerUserEndpoint');
  ...
  Future<List<Package>?> getPackages(BuildContext context) async {
    ...
    try {
      final url = Uri.parse('$baseUrl$getPackagesEndpoint');
  ...
  Future<ActivateResponse?> activatePackage({
    ...
    try {
      final url = Uri.parse('$baseUrl$activatePackageEndpoint');
      final headers = {
        'accept': '*/*', // From curl
        'Content-Type': 'application/json',
      };
 ...
 static Future<AppointmentCountResponse> getAppointmentCount({required String patientId}) async {
  ...
  Future<UserPackageResponse?> fetchUserPackage({
    ...
    try {
      final url = Uri.parse('$baseUrl/user/package?user_id=$userId');
      final headers = {
